...
public:
  ...
  std::pair<int, int> ClampSampleRange(int first, int last) const;
  ...
  void SetWaveform(const std::vector<float> &Wfm, float ZeroLevel);
  ...
  {
    return fExpNumber + 1; // fz array has fExpNumber + 1 elements (index 0 to fExpNumber)
  }
