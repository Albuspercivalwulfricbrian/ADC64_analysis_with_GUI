...
void PronyFitter::SetWaveform(const std::vector<float> &Wfm,
                              float ZeroLevel)
{
...
// void PronyFitter::SetWaveform(const std::vector<int32_t> &val_sample_vec, float zero_level)
// {
//   // Check if the vector size matches fSampleTotal
...
//   // Copy the data from vector<int32_t> to fValSampleArr (which is Float_t*)
//   for (Int_t i = 0; i < fSampleTotal; i++)
//   {
//     fValSampleArr[i] = static_cast<Float_t>(val_sample_vec[i]);
//   }
...
//     if (i > fGateBeg && i < fGateEnd)
//       std::cout << std::floor(fValSampleArr[i]) << " ";
//   }
//   std::cout << std::endl
//             << fGateBeg << " " << fGateEnd << "; zl = " << zero_level << std::endl;
//   fZeroLevel = zero_level;
// }

float PronyFitter::GoToLevel(float Level, int &point, int iterator, int iLastPoint)
{
  float ResultTime;
...
{
  fSignalBegin = (SignalBeg < 0) ? 0 : SignalBeg;
  if (fIsDebug)
...
void PronyFitter::CalculateFitAmplitudes()
{
  ...
  for (int sample_curr = 0; sample_curr < fSampleTotal; sample_curr++)
  {
    ...
    if ((sample_curr >= fSignalBegin))
    { //&& (sample_curr <= fGateEnd)) {
      for (int i = 0; i < fExpNumber + 1; i++)
      {
...
void PronyFitter::CalculateFitAmplitudesFast(int signal_length, std::complex<float> **InvHarmoMatrix)
{
  ...
  for (int i = 0; i <= fExpNumber; i++)
  {
         ...
         sample_curr++)
    {
      ...
      if (fIsDebug)
      {
        printf("%e%+ei  ", std::real(z_power[i]), std::imag(z_power[i]));
      }
      z_power[i] *= fz[i];
    }
    if (fIsDebug)
...
std::pair<int, int> PronyFitter::ClampSampleRange(int first, int last) const
{
  int n = fWfm.size();
...
{
  std::tie(first_sample, last_sample) = ClampSampleRange(first_sample, last_sample);
  int step = 0;
...
int PronyFitter::SearchSignalBeginByHarmo(int first_sample, int last_sample, int signal_length, std::complex<float> **Zpower)
{
  ...
  if (fIsDebug)
    printf("search begin in complex (1) or real (0) mode: %i\n", mode);
  ...
  for (int i = 0; i < fExpNumber + 1; i++)
  {
    ...
    if (fIsDebug)
    {
      printf("Zyk[%i] at signal begin %i :  %e%+ei  ", i, first_sample, std::real(Zyk_arr[i]), std::imag(Zyk_arr[i]));
    }
  }
  if (fIsDebug)
    printf("\n");
  ...
  if (mode == 1)
    prev_pair = std::make_pair(std::abs(std::real(Zyk_arr[1])), std::abs(std::imag(Zyk_arr[1])));
  if (mode == 0)
    prev_pair = std::make_pair(std::abs(std::real(Zyk_arr[1])), std::abs(std::real(Zyk_arr[2])));
       ...
       signal_begin++)
  {
    for (int i = 0; i < fExpNumber + 1; i++)
    {
      ...
      if (fIsDebug)
      {
        printf("Zyk[%i] at signal begin %i :  %e%+ei  ", i, signal_begin, std::real(Zyk_arr[i]), std::imag(Zyk_arr[i]));
      }
    }
    if (fIsDebug)
      printf("\n");
    ...
    if (mode == 1)
      new_pair = std::make_pair(std::abs(std::real(Zyk_arr[1])), std::abs(std::imag(Zyk_arr[1])));
    if (mode == 0)
      new_pair = std::make_pair(std::abs(std::real(Zyk_arr[1])), std::abs(std::real(Zyk_arr[2])));
    if ((new_pair.first - prev_pair.first) * (new_pair.second - prev_pair.second) < 0)
    {
      if (fIsDebug)
        printf("begin candidate at %i \n", signal_begin);
      // return signal_begin;
    }
