...
uint32_t DataFileReader::ConsequentialEventsReading(Progress *progress)
{
  ...
  while (!feof(fd) && !StopAnalysis)
  {
    ...
    if ((TotalHeader.syncword) == SYNC_WORD || (TotalHeader.syncword) == SYNC_WORD_ADC64)
    {
      ...
      if (uiTotalEvents % 10000 == 0)
        DisplayTimeToCalculate(ftell(fd) / 1024, sSizeOfFile / 1024, start_time);
      while (end > 0)
      {
        ...
        while (end > 0)
        {
          ...
          case 1:
            ...
            if (auto adcinstance = adcmap.find(TotalHeader.DeviceHeader.sn); adcinstance != adcmap.end())
              ch += 64 * adcinstance->second;
            // if (TotalHeader.DeviceHeader.sn == 101830393) ch+=64;
            event_waveform.wf.clear();
            event_waveform.ADCID = TotalHeader.DeviceHeader.sn;
            ...
            {
              event_waveform.Set_Zero_Level_Area(config_manager[ch]->leftBoundary);
            }
            else
            {
            ...
            if (config_manager[ch]->UseFourierFiltering && config_manager[ch]->FrequencyCutoff != 0)
            {
              ...
              int32_t gate = 20;
              if (gate < config_manager[ch]->leftBoundary)
                gate = config_manager[ch]->leftBoundary;
            ...
            else if (config_manager[ch] && config_manager[ch]->UseSpline == 1)
            ...
            while (true)
            {
              ...
              if (peakInfo.amp < 500 && count > 0)
                break;
              short_channel_info[ch]->AddPeak(peakInfo);
              count++;
...
//             {
//               if (counter_wf == 0)
//                 for (int sc = 0; sc < event_waveform.wf.size(); sc++)
//                   awf.push_back(0);
//               for (int sc = 0; sc < event_waveform.wf.size(); sc++)
//                 awf[sc] += (float)event_waveform.wf[sc];
//               counter_wf++;
...
void DataFileReader::setName(const char *a, const char *b)
{
  snprintf(fileName, sizeof(fileName), "%s", a);
  if (b[0])
    snprintf(configName, sizeof(configName), "%s", b);
  else
    snprintf(configName, sizeof(configName), "%s", a);
  cout << "File Name: " << fileName << "; Config File Name: " << configName << endl;
  auto cpath = std::filesystem::path(configName).parent_path().string() +
...
void DataFileReader::CreateRootFile()
{
  ...
  if (!std::filesystem::is_directory(((dirName) + "/calibrated_files/").data()))
